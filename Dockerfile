...
WORKDIR /src
COPY ["HealthRace.csproj", "."]
RUN dotnet restore "./HealthRace.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./HealthRace.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./HealthRace.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "HealthRace.dll"]